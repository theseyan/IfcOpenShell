...
from .main import Serialization, SerializedElement
...
class shape_tuple(NamedTuple):
    ...
    data: Union[SerializedElement, Serialization]
...
def create_shape_from_serialization(
    brep_object: Union[SerializedElement, Serialization],
) -> Union[shape_tuple, TopoDS.TopoDS_Shape]:
    brep_data, occ_shape, styles, style_ids = None, None, (), ()

    is_product_shape = True
    if isinstance(brep_object, SerializedElement):
        geometry = brep_object.geometry
        brep_data = geometry.brep_data
        styles = geometry.surface_styles
        style_ids = geometry.surface_style_ids
    elif isinstance(brep_object, Serialization):
        try:

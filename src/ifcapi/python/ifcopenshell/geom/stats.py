...
"""Streaming geometry statistics over the native C ABI."""
...
class StatsCollector:
    streamer: ifcopenshell._InstanceStreamer
    ...
    def __init__(self):
        self.streamer = ifcopenshell._InstanceStreamer.create()
    ...
    def _declaration_for_record(self, inst: dict):
        type_name = inst["type"]
        if type_name == "file_schema":
            identifiers = inst.get("schema_identifiers") or []
            if identifiers:
                self.schema = ifcopenshell_wrapper.schema_by_name(identifiers[0])
            return None
        if self.schema is None or type_name.startswith("file_"):
            return None
        return self.schema.declaration_by_name(type_name)

    def next(self):
        if self.num_semis > 0:
            inst = self.streamer.readInstancePy()
            self.num_semis -= 1
            if inst:
                decl = self._declaration_for_record(inst)
                if decl is None:
                    return None
                return decl, dict(list(inst.items())[2:])
            self.finalized = True
            return None
        if self.file_stream:
            self.feedFromFile()

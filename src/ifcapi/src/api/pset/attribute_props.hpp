...
inline void apply_attribute_prop(express::Base entity, const ifcapi_pset::Entry& entry) {
    if (!entity) return;
    ...
    switch (entry.kind) {
        case ifcapi_pset::Kind::NONE:
            clear_attr(entity, entry.key.c_str());
            break;
...
inline void apply_attribute_props(express::Base entity, ifcopenshell_pset_props_t* props) {
    if (!props) return;

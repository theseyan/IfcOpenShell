...
void set_double_vec(express::Base e, const char* attr, const std::vector<double>& v) {
    int i = find_attr_idx(e, attr);
    if (i < 0) return;
    e.set_attribute_value(static_cast<size_t>(i), v);
}

size_t total_inverses(ifcopenshell::file* file, express::Base e) {
...
express::Base find_relating(
    ifcopenshell::file* file, express::Base product,
...
express::Base compute_placement_rel_to(ifcopenshell::file* file, express::Base product) {
...
express::Base create_entity(ifcopenshell::file* file, const char* type_name) {
...
std::vector<ChildJob> get_children_settings(
    ifcopenshell::file* file, express::Base old_placement)
{
...
express::Base edit_placement_impl(
...
{
    if (!has_attr(product, "ObjectPlacement")) return {};
    ...
    express::Base old_placement = get_entity_ref(product, "ObjectPlacement");
    ...
    express::Base placement_rel_to = compute_placement_rel_to(file, product);

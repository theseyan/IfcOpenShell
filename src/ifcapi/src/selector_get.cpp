 ...
 *  Children in list_val / dict_val are owned and freed recursively.
 * ==================================================================== */
...
struct ifcopenshell_value_t {
    ...
    double  d_val  = 0.0;
    std::string s_val;
    IfcUtil::IfcBaseClass* inst_val = nullptr; /* NOT owned */

    std::vector<ifcopenshell_value_t*> list_val;                        /* owned */
    std::vector<std::pair<std::string, ifcopenshell_value_t*>> dict_val; /* owned */
...
/* ---- RAII scoped handle — wraps an entity in a temporary handle without taking ownership. */
struct ScopedHandle {
...
static bool entity_is_a(IfcUtil::IfcBaseClass* e, const char* type) {
    return e && e->declaration().is(type);
}

static int find_attr_idx(IfcUtil::IfcBaseClass* e, const char* name) {
    auto* decl = e->declaration().as_entity();
    if (!decl) return -1;
    int idx = static_cast<int>(decl->attribute_index(name));
    return idx;
}

static std::string get_string_attr(IfcUtil::IfcBaseClass* e, const char* attr) {
...
static IfcUtil::IfcBaseClass* get_entity_ref(IfcUtil::IfcBaseClass* e, const char* attr) {
    int idx = find_attr_idx(e, attr);
...
static std::vector<IfcUtil::IfcBaseClass*> get_entity_list(IfcUtil::IfcBaseClass* e, const char* attr) {
...
static std::vector<IfcUtil::IfcBaseClass*> get_inverse_list(IfcUtil::IfcBaseClass* e, const char* name) {
    std::vector<IfcUtil::IfcBaseClass*> result;
...
/* ---- Typed-value unwrapping (IfcLabel, IfcReal, etc.) ---- */

static Val* unwrap_typed_value(IfcUtil::IfcBaseClass* typed) {
...
/* Convert an AttributeValue to a Val*, unwrapping typed instances. */
static Val* attr_to_val(const AttributeValue& v) {
...
/* Read a named attribute from entity; returns nullptr if attribute absent
   (distinct from make_none() which means attribute present but null). */
static Val* read_attr(IfcUtil::IfcBaseClass* e, const std::string& attr_name) {
    auto* decl = e->declaration().as_entity();
...
static Val* extract_prop_value(IfcUtil::IfcBaseClass* prop) {
...
/* Extract all properties/quantities from a single pset definition entity.
   Returns a DICT Val* (without the "id" key — Python removes it). */
static Val* extract_pset_props(IfcUtil::IfcBaseClass* defn) {
...
/* Return all psets for an entity as a vector of (name, DICT Val*) pairs.
   Same-name psets are merged. Caller owns all Val* values. */
static std::vector<std::pair<std::string, Val*>>
...
struct KeyEntry {
    bool is_regex = false;
    std::string text;
    std::regex pattern; /* valid only if is_regex */
};

static std::string unescape_quoted(const std::string& raw) {
    /* raw is the full ESCAPED_STRING token text including outer quotes.
       Python: args[1:-1].replace("\\", "")  */
...
static std::vector<KeyEntry> extract_keys(const ifcopenshell_selector_node_t* root) {
...
static Val* resolve_predefined_type(IfcUtil::IfcBaseClass* e) {
    /* Mirrors ifcopenshell.util.element.get_predefined_type */
    ScopedHandle sh(e);
...
static Val* resolve_classification(IfcUtil::IfcBaseClass* e) {
    auto* list = make_list();
...
static Val* resolve_groups(IfcUtil::IfcBaseClass* e) {
    auto* list = make_list();
...
static Val* resolve_occurrences(IfcUtil::IfcBaseClass* e) {
    auto* list = make_list();
...
static Val* resolve_xyz(IfcUtil::IfcBaseClass* e, const std::string& k) {
    ...
    /* Column-major 4x4: translation is matrix[0*4+3], [1*4+3], [2*4+3] */
    int ci = (k == "x") ? 0 : (k == "y") ? 1 : 2;
    return make_double(matrix[ci * 4 + 3]);
}

/* ---- Regex helper: Python re.match equivalent (anchored at start) ---- */
static bool regex_match_start(const std::string& str, const std::regex& pattern) {
    std::smatch m;
    return std::regex_search(str, m, pattern, std::regex_constants::match_continuous);
}

/* ---- Core apply_key logic ---- */

/* Forward declaration for mutual recursion with the list branch. */
static Val* apply_key(IfcParse::IfcFile* file, const Val* cur, const KeyEntry& key);

static Val* apply_key_to_list(IfcParse::IfcFile* file, const Val* cur, const KeyEntry& key) {
    ...
    /* Non-numeric: apply key to each element, collect results */
...
static Val* apply_key(IfcParse::IfcFile* file, const Val* cur, const KeyEntry& key) {
    ...
    /* ----------------------------------------------------------------
     *  DICT  (pset result from a prior key)
     * ---------------------------------------------------------------- */
    ...
    if (!key.is_regex) {
        ...
        /* easting / northing / elevation — geolocation not yet ported */
        if ((k == "easting" || k == "northing" || k == "elevation") &&
            find_attr_idx(e, "ObjectPlacement") >= 0) {
            return make_none();
        }

        /* ---- IfcMaterialLayerSet.Name → LayerSetName alias ---- */
    ...
    /* ---- Pset lookup (plain name or regex) ---- */
    ...
    /* Python: `if result: del result["id"]` — we never add "id",
       but an empty dict (no properties) maps to Python's {} which is
       falsy; preserve the empty dict so subsequent key lookups get None. */
    return found_pset;
}

/* ---- Top-level evaluator ---- */

static Val* get_element_value_impl(
    ...
    const char* query)
{
    ...
    if (!ast) return nullptr; /* error already set by parser */

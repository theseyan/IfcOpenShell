...
from ifcopenshell.api.geometry.add_window_representation import DEFAULT_PANEL_SCHEMAS, WindowPanelProperties
...
class TestAddWindowRepresentation(test.bootstrap.IFC4, WindowRepresentationContext):
    def test_creates_expected_representations_for_all_view_modes(self):
        expected = {
            ...
            "MODEL_VIEW": (
                "SweptSolid",
                4,
                ["IfcExtrudedAreaSolid", "IfcExtrudedAreaSolid", "IfcExtrudedAreaSolid", "IfcExtrudedAreaSolid"],
            ),
        }

        for target_view, (representation_type, item_count, item_types) in expected.items():
            context = self.body_context(target_view)
            representation = ifcopenshell.api.geometry.add_window_representation(self.file, context=context)
    ...
    def test_adds_lining_framing_and_glazing_shape_aspects(self):
        ...
        assert all(aspect.ShapeRepresentations[0].RepresentationType == representation.RepresentationType for aspect in aspects.values())

    def test_mutates_panel_property_inputs_to_initialised_dictionaries(self):
        context = self.body_context()
        panel_properties = [WindowPanelProperties(FrameDepth=0.04), {"FrameThickness": 0.02}]
        ...
        assert panel_properties == [
            {"FrameDepth": 0.04, "FrameThickness": 0.035, "PanelPosition": None, "PanelOperation": None, "ShapeAspectStyle": None},
            {"FrameDepth": 0.035, "FrameThickness": 0.02, "PanelPosition": None, "PanelOperation": None, "ShapeAspectStyle": None},
        ]
    ...
    def test_rejects_invalid_partition_type_like_upstream(self):
        ...
        with pytest.raises(KeyError):
            ifcopenshell.api.geometry.add_window_representation(self.file, context=context, partition_type="INVALID")

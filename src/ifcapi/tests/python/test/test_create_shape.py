...
class TestSerializerFactories:
    def test_obj_stream_outputs(self):
        geometry_settings = ifcopenshell.geom.settings()
        serializer_settings = ifcopenshell.geom.serializer_settings()
        obj_output = ifcopenshell.geom.serializers.buffer()
        mtl_output = ifcopenshell.geom.serializers.buffer()

        serializer = ifcopenshell.geom.serializers.obj(
            obj_output, mtl_output, geometry_settings, serializer_settings
        )
        serializer.writeHeader()

        assert serializer.ready()
        assert obj_output.get_value().startswith("# File generated by IfcOpenShell")
        assert mtl_output.get_value().startswith("# File generated by IfcOpenShell")

    @pytest.mark.parametrize("factory", [ifcopenshell.geom.serializers.svg, ifcopenshell.geom.serializers.ttl])
    def test_single_stream_output(self, factory):
        output = ifcopenshell.geom.serializers.buffer()
        serializer = factory(
            output, ifcopenshell.geom.settings(), ifcopenshell.geom.serializer_settings()
        )

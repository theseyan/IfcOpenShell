...
def render_python_extension(metadata: BindingABI) -> str:
    ...
    error_constants = "\n".join(
        f'    PyModule_AddIntConstant(m, "IFCOPENSHELL_ERROR_{entry.name}", {entry.value});'
        for entry in metadata.error_catalog.kinds
    )
    error_code_constants = "\n".join(
        f'    PyModule_AddIntConstant(m, "IFCOPENSHELL_ERROR_CODE_{entry.name}", {entry.value});'
        for entry in metadata.error_catalog.codes
    )
    return f"""\
...
static PyObject *raise_last_error(const char *fallback) {{
    ...
    int code = {metadata.error_functions["last_error_code"]}();
    PyObject *exc;
    switch (kind) {{
...
static PyObject *py_last_error_code(PyObject *self, PyObject *args) {{
    return PyLong_FromLong({metadata.error_functions["last_error_code"]}());
}}

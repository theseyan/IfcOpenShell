"""Generate the complete IfcOpenShell binding surface in one process."""
...
from .targets.python.backend import render_capi_utils, render_python_extension
...
class ProductionSourceSet:
    ...
    def from_spec_dir(cls, spec_dir: Path) -> ProductionSourceSet:
        repo_root = spec_dir.resolve().parents[3]
        return cls(
            ...
            binding_headers=tuple(
                sorted((repo_root / "src/ifcapi/include/ifcapi/bindings").glob("*.h"))
            ),
            cpp_namespaces=(
                "ifcparse::bindings",
                "ifcapi::bindings",
            ...
            handle_prefixes=("ifcopenshell", "ifcopenshell", "ifcopenshell_geom"),
        )
...
class GenerationConfig:
    ...
    python_utils_out: Path
    wasm_output_dir: Path
    js_generated_output_dir: Path
...
def generate_all(config: GenerationConfig) -> tuple[Artifact, ...]:
    ...
    python_dir = config.python_output_dir.resolve()
    wasm_dir = config.wasm_output_dir.resolve()
    package_dir = config.js_generated_output_dir.resolve()
    ...
    debug_log("pipeline.emit.python", f"ir={id(ir)} abi={id(metadata)}")
    python_extension = render_python_extension(metadata)
    python_utils = render_capi_utils()
    debug_log("pipeline.emit.wasm", f"ir={id(ir)} abi={id(metadata)}")
    javascript, declarations = render_wasm_bindings(metadata)
    direct_api = render_api_direct(metadata)
    exports = render_export_list(metadata)
    ...
    artifacts = (
        ...
        Artifact("wasm-api", wasm_dir / "ifcopenshell_api.ts", direct_api),
        Artifact(
            "package-declarations", package_dir / "ifcopenshell_api.d.ts", declarations
        ),
        Artifact("package-api", package_dir / "ifcopenshell_api.ts", direct_api),
    )
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--python-output-dir", type=Path, required=True)
    parser.add_argument("--python-utils-out", type=Path, required=True)
    parser.add_argument("--wasm-output-dir", type=Path, required=True)
    parser.add_argument("--js-generated-output-dir", type=Path, required=True)
    parser.add_argument(
...
def main() -> int:
    ...
    generate_all(
        GenerationConfig(
            ...
            python_output_dir=args.python_output_dir,
            python_utils_out=args.python_utils_out,
            wasm_output_dir=args.wasm_output_dir,
            js_generated_output_dir=args.js_generated_output_dir,

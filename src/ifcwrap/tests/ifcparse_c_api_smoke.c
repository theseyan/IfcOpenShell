...
static void test_core(void) {
    ...
    ifcopenshell_ifc_instance_list_t traversed_instances_value = {0};
    ifcopenshell_ifc_instance_list_t traversed_instances_bfs_value = {0};
    ifcopenshell_ifc_instance_list_t instances_by_type = {0};
    ifcopenshell_ifc_instance_list_t instances_by_type_excl_subtypes = {0};
    ifcopenshell_ifc_instance_list_t instances_by_reference = {0};
    ...
    expect_true(ifcopenshell_ifcparse_schema_names(&schema_names), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_schema_name(schema_lookup, &schema_lookup_name), ifcopenshell_last_error_message());
    expect_true(strcmp(schema_lookup_name.data, "IFC4") == 0, "Unexpected looked up schema name");
    expect_true(ifcopenshell_ifc_schema_declarations(schema_lookup, &schema_declarations), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_declaration_name(schema_declarations.items[0], &declaration_name), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_schema_enumeration_types(schema_lookup, &schema_enumeration_types), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_schema_select_types(schema_lookup, &schema_select_types), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_schema_type_declarations(schema_lookup, &schema_type_declarations), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_schema_entities(schema_lookup, &schema_entities), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_file_by_id(file, 1u, &point), ifcopenshell_last_error_message());
    expect_true(point != NULL, "Point instance handle is null");
    expect_true(ifcopenshell_ifc_file_by_id(file, 2u, &point_two), ifcopenshell_last_error_message());
    expect_true(point_two != NULL, "Second point instance handle is null");
    expect_true(ifcopenshell_ifc_file_by_type(file, "IfcCartesianPoint", &instances_by_type), ifcopenshell_last_error_message());
    expect_true(instances_by_type.size == 2u, "Unexpected instances-by-type size");
    expect_true(ifcopenshell_ifc_file_by_type_excl_subtypes(file, "IfcCartesianPoint", &instances_by_type_excl_subtypes), ifcopenshell_last_error_message());
    expect_true(instances_by_type_excl_subtypes.size == 2u, "Unexpected instances-by-type-excl-subtypes size");
    expect_true(ifcopenshell_ifc_file_instances_by_reference(file, 1u, &instances_by_reference), ifcopenshell_last_error_message());
    expect_true(instances_by_reference.size == 1u, "Unexpected instances-by-reference size");

    expect_true(ifcopenshell_ifc_file_by_id(file, 3u, &polyline), ifcopenshell_last_error_message());
    expect_true(polyline != NULL, "Polyline instance handle is null");
    expect_true(ifcopenshell_ifc_file_by_id(file, 4u, &person), ifcopenshell_last_error_message());
    expect_true(person != NULL, "Person instance handle is null");
    expect_true(ifcopenshell_ifc_file_by_id(file, 5u, &colour), ifcopenshell_last_error_message());
    expect_true(colour != NULL, "Colour instance handle is null");
    expect_true(ifcopenshell_ifc_file_by_id(file, 6u, &indexed_polygonal_face), ifcopenshell_last_error_message());
    expect_true(indexed_polygonal_face != NULL, "Indexed polygonal face instance handle is null");
    expect_true(ifcopenshell_ifc_file_by_id(file, 7u, &texture_vertex_list), ifcopenshell_last_error_message());
    expect_true(texture_vertex_list != NULL, "Texture vertex list instance handle is null");
    expect_true(ifcopenshell_ifc_file_by_id(file, 8u, &indexed_polygonal_face_with_voids), ifcopenshell_last_error_message());
    expect_true(indexed_polygonal_face_with_voids != NULL, "Indexed polygonal face with voids instance handle is null");

    expect_true(ifcopenshell_ifc_file_good(file, &file_status), ifcopenshell_last_error_message());
    expect_true(file_status == 0, "Unexpected file open status");
    expect_true(ifcopenshell_ifc_file_to_string(file, &file_spf), ifcopenshell_last_error_message());
    expect_true(strstr(file_spf.data, "ISO-10303-21") != NULL, "File SPF output missing header");
    expect_true(ifcopenshell_ifc_file_entity_names(file, &entity_ids), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_file_file_pointer(file, &file_pointer), ifcopenshell_last_error_message());
    expect_true(file_pointer != 0u, "Unexpected file pointer value");
    expect_true(ifcopenshell_ifc_file_storage_mode(file, &file_storage_mode), ifcopenshell_last_error_message());
    expect_true(file_storage_mode == 0, "Unexpected file storage mode");
    expect_true(ifcopenshell_ifcparse_guess_file_type("smoke.ifc", &guessed_file_type), ifcopenshell_last_error_message());
    expect_true(guessed_file_type == 0, "Unexpected guessed file type");

    expect_true(ifcopenshell_ifc_file_header(file, &header), ifcopenshell_last_error_message());
    expect_true(header != NULL, "Header handle is null");
    expect_true(ifcopenshell_ifc_header_file_description(header, &file_description), ifcopenshell_last_error_message());
    expect_true(file_description != NULL, "File description handle is null");
    expect_true(ifcopenshell_ifc_header_file_name(header, &file_name), ifcopenshell_last_error_message());
    expect_true(file_name != NULL, "File name handle is null");
    expect_true(ifcopenshell_ifc_header_file_schema(header, &file_schema), ifcopenshell_last_error_message());
    expect_true(file_schema != NULL, "File schema handle is null");
    expect_true(ifcopenshell_ifc_file_description_description(file_description, &header_description), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_file_name_author(file_name, &header_authors), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_file_schema_schema_identifiers(file_schema, &header_schema_identifiers), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_file_schema(file, &schema), ifcopenshell_last_error_message());
    expect_true(schema != NULL, "Schema handle is null");

    expect_true(ifcopenshell_ifc_file_get_max_id(file, &max_id), ifcopenshell_last_error_message());
    expect_true(max_id == 9u, "Unexpected max instance id");

    expect_true(ifcopenshell_ifc_instance_id(point, &instance_id), ifcopenshell_last_error_message());
    expect_true(instance_id == 1u, "Unexpected instance id");

    expect_true(ifcopenshell_ifc_instance_identity(point, &instance_identity), ifcopenshell_last_error_message());
    expect_true(instance_identity != 0u, "Unexpected instance identity");
    expect_true(ifcopenshell_ifc_instance_file_pointer(point, &instance_file_pointer), ifcopenshell_last_error_message());
    expect_true(instance_file_pointer == file_pointer, "Unexpected instance file pointer");
    expect_true(ifcopenshell_ifc_instance_get_argument_index(point, "Coordinates", &argument_index), ifcopenshell_last_error_message());
    expect_true(argument_index == 0u, "Unexpected argument index");
    expect_true(ifcopenshell_ifc_instance_get_argument_name(point, 0u, &argument_name), ifcopenshell_last_error_message());
    expect_true(strcmp(argument_name.data, "Coordinates") == 0, "Unexpected argument name");

    expect_true(ifcopenshell_ifc_instance_declaration(point, &declaration), ifcopenshell_last_error_message());
    expect_true(declaration != NULL, "Declaration handle is null");
    expect_true(ifcopenshell_ifc_schema_declaration_by_name(schema_lookup, "IfcCartesianPoint", &declaration_by_name), ifcopenshell_last_error_message());
    expect_true(declaration_by_name != NULL, "Schema declaration lookup by name returned null");
    expect_true(ifcopenshell_ifc_schema_declaration_by_name(schema_lookup, "IfcActionRequestTypeEnum", &enum_declaration), ifcopenshell_last_error_message());
    expect_true(enum_declaration != NULL, "Enum declaration lookup by name returned null");
    expect_true(ifcopenshell_ifc_declaration_name(declaration, &declaration_name), ifcopenshell_last_error_message());
    expect_true(strcmp(declaration_name.data, "IfcCartesianPoint") == 0, "Unexpected declaration name");
    expect_true(ifcopenshell_ifc_declaration_name_uc(declaration, &declaration_name_uc), ifcopenshell_last_error_message());
    expect_true(strcmp(declaration_name_uc.data, "IFCCARTESIANPOINT") == 0, "Unexpected upper-case declaration name");
    expect_true(ifcopenshell_ifc_declaration_schema(declaration, &declaration_schema), ifcopenshell_last_error_message());
    expect_true(declaration_schema != NULL, "Declaration schema handle is null");
    expect_true(ifcopenshell_ifc_schema_name(declaration_schema, &declaration_schema_name), ifcopenshell_last_error_message());
    expect_true(strcmp(declaration_schema_name.data, "IFC4") == 0, "Unexpected declaration schema name");
    expect_true(ifcopenshell_ifc_declaration_index_in_schema(declaration, &declaration_index), ifcopenshell_last_error_message());
    expect_true(declaration_index >= 0, "Unexpected declaration index");
    expect_true(ifcopenshell_ifc_declaration_type(declaration, &declaration_type), ifcopenshell_last_error_message());
    expect_true(declaration_type == declaration_index, "Unexpected declaration type value");
    expect_true(ifcopenshell_ifc_schema_declaration_by_index(schema_lookup, (size_t)declaration_index, &declaration_by_index), ifcopenshell_last_error_message());
    expect_true(declaration_by_index != NULL, "Schema declaration lookup by index returned null");
    expect_true(ifcopenshell_ifc_declaration_is_a(declaration, "IfcCartesianPoint", &declaration_is_a), ifcopenshell_last_error_message());
    expect_true(declaration_is_a, "Declaration type check failed");
    expect_true(ifcopenshell_ifc_declaration_as_entity(declaration, &entity), ifcopenshell_last_error_message());
    expect_true(entity != NULL, "Entity cast returned null");
    expect_true(ifcopenshell_ifc_entity_attribute_count(entity, &entity_attribute_count), ifcopenshell_last_error_message());
    expect_true(entity_attribute_count == 1u, "Unexpected entity attribute count");
    expect_true(ifcopenshell_ifc_entity_is_abstract(entity, &entity_is_abstract), ifcopenshell_last_error_message());
    expect_true(!entity_is_abstract, "IfcCartesianPoint should not be abstract");
    expect_true(ifcopenshell_ifc_entity_derived(entity, &entity_derived), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_entity_argument_types(entity, &entity_argument_types), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_entity_all_attributes(entity, &entity_attributes), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_attribute_name(attribute, &attribute_name), ifcopenshell_last_error_message());
    expect_true(strcmp(attribute_name.data, "Coordinates") == 0, "Unexpected entity attribute name");
    expect_true(ifcopenshell_ifc_attribute_optional(attribute, &attribute_optional), ifcopenshell_last_error_message());
    expect_true(!attribute_optional, "IfcCartesianPoint.Coordinates should not be optional");
    expect_true(ifcopenshell_ifc_attribute_type_of_attribute(attribute, &element_parameter_type), ifcopenshell_last_error_message());
    expect_true(element_parameter_type != NULL, "Attribute parameter type is null");
    expect_true(ifcopenshell_ifc_declaration_as_enumeration_type(enum_declaration, &enumeration), ifcopenshell_last_error_message());
    expect_true(enumeration != NULL, "Enumeration cast returned null");
    expect_true(ifcopenshell_ifc_enumeration_argument_types(enumeration, &enumeration_argument_types), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_enumeration_enumeration_items(enumeration, &enumeration_items), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_enumeration_lookup_enum_value(enumeration, 0u, &enum_value), ifcopenshell_last_error_message());
    expect_true(strcmp(enum_value.data, "EMAIL") == 0, "Unexpected enumeration value");
    expect_true(ifcopenshell_ifc_enumeration_lookup_enum_offset(enumeration, "EMAIL", &enum_offset), ifcopenshell_last_error_message());
    expect_true(enum_offset == 0u, "Unexpected enumeration offset");
    expect_true(ifcopenshell_ifc_schema_declaration_by_name(schema_lookup, "IfcLineIndex", &line_index_declaration), ifcopenshell_last_error_message());
    expect_true(line_index_declaration != NULL, "Type declaration lookup by name returned null");
    expect_true(ifcopenshell_ifc_declaration_as_type_declaration(line_index_declaration, &type_declaration), ifcopenshell_last_error_message());
    expect_true(type_declaration != NULL, "Type declaration cast returned null");
    expect_true(ifcopenshell_ifc_type_declaration_argument_types(type_declaration, &type_declaration_argument_types), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_type_declaration_declared_type(type_declaration, &parameter_type), ifcopenshell_last_error_message());
    expect_true(parameter_type != NULL, "Declared parameter type returned null");
    expect_true(ifcopenshell_ifc_parameter_type_as_aggregation_type(parameter_type, &aggregation_type), ifcopenshell_last_error_message());
    expect_true(aggregation_type != NULL, "Aggregation type cast returned null");
    expect_true(ifcopenshell_ifc_aggregation_type_bound1(aggregation_type, &aggregation_bound1), ifcopenshell_last_error_message());
    expect_true(aggregation_bound1 == 2, "Unexpected aggregation lower bound");
    expect_true(ifcopenshell_ifc_aggregation_type_bound2(aggregation_type, &aggregation_bound2), ifcopenshell_last_error_message());
    expect_true(aggregation_bound2 == -1, "Unexpected aggregation upper bound");
    expect_true(ifcopenshell_ifc_aggregation_type_type_of_aggregation(aggregation_type, &aggregation_type_value), ifcopenshell_last_error_message());
    expect_true(aggregation_type_value == 2, "Unexpected aggregation enum value");
    expect_true(ifcopenshell_ifc_aggregation_type_type_of_aggregation_string(aggregation_type, &aggregation_type_name), ifcopenshell_last_error_message());
    expect_true(strcmp(aggregation_type_name.data, "list") == 0, "Unexpected aggregation type string");
    expect_true(ifcopenshell_ifc_aggregation_type_kind(aggregation_type, &aggregation_kind), ifcopenshell_last_error_message());
    expect_true(strcmp(aggregation_kind.data, "LIST") == 0, "Unexpected aggregation kind");
    expect_true(ifcopenshell_ifc_aggregation_type_type_of_element(aggregation_type, &element_parameter_type), ifcopenshell_last_error_message());
    expect_true(element_parameter_type != NULL, "Aggregation element type returned null");
    expect_true(ifcopenshell_ifc_schema_declaration_by_name(schema_lookup, "IfcSegmentIndexSelect", &segment_index_select_declaration), ifcopenshell_last_error_message());
    expect_true(segment_index_select_declaration != NULL, "Select declaration lookup by name returned null");
    expect_true(ifcopenshell_ifc_declaration_as_select_type(segment_index_select_declaration, &select_type), ifcopenshell_last_error_message());
    expect_true(select_type != NULL, "Select type cast returned null");
    expect_true(ifcopenshell_ifc_select_type_select_list_names(select_type, &select_list_names), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_schema_declaration_by_name(schema_lookup, "IfcPositiveInteger", &positive_integer_declaration), ifcopenshell_last_error_message());
    expect_true(positive_integer_declaration != NULL, "Named declaration lookup by name returned null");
    expect_true(ifcopenshell_ifc_declaration_as_type_declaration(positive_integer_declaration, &positive_integer_type_declaration), ifcopenshell_last_error_message());
    expect_true(positive_integer_type_declaration != NULL, "Named type declaration cast returned null");
    expect_true(ifcopenshell_ifc_type_declaration_declared_type(positive_integer_type_declaration, &named_parameter_type), ifcopenshell_last_error_message());
    expect_true(named_parameter_type != NULL, "Named parameter type returned null");
    expect_true(ifcopenshell_ifc_parameter_type_kind(named_parameter_type, &parameter_kind), ifcopenshell_last_error_message());
    expect_true(strcmp(parameter_kind.data, "NAMED") == 0, "Unexpected parameter type kind for IfcPositiveInteger");
    expect_true(ifcopenshell_ifc_parameter_type_as_named_type(named_parameter_type, &named_type), ifcopenshell_last_error_message());
    expect_true(named_type != NULL, "Named type cast returned null");
    expect_true(ifcopenshell_ifc_named_type_declared_type(named_type, &named_declaration), ifcopenshell_last_error_message());
    expect_true(named_declaration != NULL, "Named type declaration target returned null");
    expect_true(ifcopenshell_ifc_declaration_name(named_declaration, &named_declaration_name), ifcopenshell_last_error_message());
    expect_true(strcmp(named_declaration_name.data, "IfcInteger") == 0, "Unexpected named type target");
    expect_true(ifcopenshell_ifc_schema_declaration_by_name(schema_lookup, "IfcIdentifier", &identifier_declaration), ifcopenshell_last_error_message());
    expect_true(identifier_declaration != NULL, "Simple declaration lookup by name returned null");
    expect_true(ifcopenshell_ifc_declaration_as_type_declaration(identifier_declaration, &identifier_type_declaration), ifcopenshell_last_error_message());
    expect_true(identifier_type_declaration != NULL, "Simple type declaration cast returned null");
    expect_true(ifcopenshell_ifc_type_declaration_declared_type(identifier_type_declaration, &simple_parameter_type), ifcopenshell_last_error_message());
    expect_true(simple_parameter_type != NULL, "Simple parameter type returned null");
    expect_true(ifcopenshell_ifc_parameter_type_kind(simple_parameter_type, &parameter_kind), ifcopenshell_last_error_message());
    expect_true(strcmp(parameter_kind.data, "SIMPLE") == 0, "Unexpected parameter type kind for IfcIdentifier");
    expect_true(ifcopenshell_ifc_parameter_type_as_simple_type(simple_parameter_type, &simple_type), ifcopenshell_last_error_message());
    expect_true(simple_type != NULL, "Simple type cast returned null");
    expect_true(ifcopenshell_ifc_simple_type_kind(simple_type, &simple_kind), ifcopenshell_last_error_message());
    expect_true(strcmp(simple_kind.data, "STRING") == 0, "Unexpected simple type kind");

    expect_true(ifcopenshell_ifc_file_get_total_inverses_by_id(file, 3, &total_inverses), ifcopenshell_last_error_message());
    expect_true(total_inverses == 0u, "Unexpected inverse count by id");
    expect_true(ifcopenshell_ifc_file_get_total_inverses(file, point, &total_inverses_for_instance), ifcopenshell_last_error_message());
    expect_true(total_inverses_for_instance == 1, "Unexpected inverse count for point instance");
    expect_true(ifcopenshell_ifc_file_get_inverse(file, point, &inverse_instances), ifcopenshell_last_error_message());
    expect_true(inverse_instances != NULL, "Inverse instance list is null");
    expect_true(ifcopenshell_ifcparse_instance_list_size(inverse_instances, &attribute_size), ifcopenshell_last_error_message());
    expect_true(attribute_size == 1u, "Unexpected inverse instance list size");
    expect_true(ifcopenshell_ifcparse_instance_list_get(inverse_instances, 0u, &inverse_instance), ifcopenshell_last_error_message());
    expect_true(inverse_instance != NULL, "Inverse instance is null");
    expect_true(ifcopenshell_ifc_instance_class_name(inverse_instance, false, &class_name), ifcopenshell_last_error_message());
    expect_true(strcmp(class_name.data, "IfcPolyline") == 0, "Unexpected inverse instance class");
    ifcopenshell_string_destroy(&class_name);

    expect_true(ifcopenshell_ifc_file_types(file, &file_types), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_instance_get_argument_type(point, 0u, &argument_type_name), ifcopenshell_last_error_message());
    expect_true(strcmp(argument_type_name.data, "AGGREGATE OF DOUBLE") == 0, "Unexpected point argument type");
    expect_true(ifcopenshell_ifc_instance_to_string(point, true, &instance_spf), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_instance_get_attribute_names(person, &attribute_names), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_instance_get_attribute_category(person, "FamilyName", &declaration_type), ifcopenshell_last_error_message());
    expect_true(declaration_type == 1, "Unexpected attribute category for FamilyName");
    expect_true(ifcopenshell_ifc_instance_get_inverse_attribute_names(point, &inverse_attribute_names), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_instance_get_attribute_category(point, inverse_attribute_names.items[0].data, &declaration_type), ifcopenshell_last_error_message());
    expect_true(declaration_type == 2, "Unexpected attribute category for inverse attribute");
    expect_true(ifcopenshell_ifc_instance_get_inverse(point, inverse_attribute_names.items[0].data, &traversed_instances_bfs), ifcopenshell_last_error_message());
    expect_true(traversed_instances_bfs != NULL, "Inverse lookup result is null");
    expect_true(ifcopenshell_ifcparse_instance_list_size(traversed_instances_bfs, &attribute_size), ifcopenshell_last_error_message());
    ifcopenshell_ifcparse_instance_list_destroy(traversed_instances_bfs);
    traversed_instances_bfs = NULL;
    expect_true(ifcopenshell_ifc_file_get_inverse_indices(file, point, &indexed_face_index_values), ifcopenshell_last_error_message());
    expect_true(indexed_face_index_values.size == 1u, "Unexpected inverse index list size");
    expect_true(ifcopenshell_ifc_file_get_inverse_indices_by_id(file, 1, &inverse_index_values_by_id), ifcopenshell_last_error_message());
    expect_true(inverse_index_values_by_id.size == 1u, "Unexpected inverse index-by-id list size");

    expect_true(ifcopenshell_ifc_instance_get_argument(point_two, 0u, &point_coordinates), ifcopenshell_last_error_message());
    expect_true(point_coordinates != NULL, "Point coordinate attribute value handle is null");
    expect_true(ifcopenshell_ifcparse_attribute_value_is_null(point_coordinates, &attribute_is_null), ifcopenshell_last_error_message());
    expect_true(!attribute_is_null, "Point coordinate attribute unexpectedly null");
    expect_true(ifcopenshell_ifcparse_attribute_value_type(point_coordinates, &attribute_type), ifcopenshell_last_error_message());
    expect_true(strcmp(attribute_type.data, "AGGREGATE OF DOUBLE") == 0, "Unexpected point coordinate attribute type");
    expect_true(ifcopenshell_ifcparse_attribute_value_size(point_coordinates, &attribute_size), ifcopenshell_last_error_message());
    expect_true(attribute_size == 3u, "Unexpected point coordinate attribute size");
    expect_true(ifcopenshell_ifcparse_attribute_value_as_double_list(point_coordinates, &point_coordinate_values), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_instance_get_argument(polyline, 0u, &polyline_points), ifcopenshell_last_error_message());
    expect_true(polyline_points != NULL, "Polyline points attribute value handle is null");
    expect_true(ifcopenshell_ifcparse_attribute_value_type(polyline_points, &attribute_type), ifcopenshell_last_error_message());
    expect_true(strcmp(attribute_type.data, "AGGREGATE OF ENTITY INSTANCE") == 0, "Unexpected polyline points attribute type");
    expect_true(ifcopenshell_ifcparse_attribute_value_size(polyline_points, &attribute_size), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_instance_get_argument(person, 1u, &family_name_argument), ifcopenshell_last_error_message());
    expect_true(family_name_argument != NULL, "Person family name attribute value handle is null");
    expect_true(ifcopenshell_ifcparse_attribute_value_type(family_name_argument, &attribute_type), ifcopenshell_last_error_message());
    expect_true(strcmp(attribute_type.data, "STRING") == 0, "Unexpected family name attribute type");
    expect_true(ifcopenshell_ifcparse_attribute_value_as_string(family_name_argument, &family_name), ifcopenshell_last_error_message());
    expect_true(strcmp(family_name.data, "Doe") == 0, "Unexpected family name value");
    ifcopenshell_string_destroy(&family_name);
    ifcopenshell_string_destroy(&attribute_type);

    expect_true(ifcopenshell_ifc_instance_get_argument(person, 3u, &middle_names_argument), ifcopenshell_last_error_message());
    expect_true(middle_names_argument != NULL, "Person middle names attribute value handle is null");
    expect_true(ifcopenshell_ifcparse_attribute_value_type(middle_names_argument, &attribute_type), ifcopenshell_last_error_message());
    expect_true(strcmp(attribute_type.data, "AGGREGATE OF STRING") == 0, "Unexpected middle names attribute type");
    expect_true(ifcopenshell_ifcparse_attribute_value_as_string_list(middle_names_argument, &middle_name_values), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_instance_get_argument(indexed_polygonal_face, 0u, &indexed_face_indices), ifcopenshell_last_error_message());
    expect_true(indexed_face_indices != NULL, "Indexed polygonal face coordinate index handle is null");
    expect_true(ifcopenshell_ifcparse_attribute_value_type(indexed_face_indices, &attribute_type), ifcopenshell_last_error_message());
    expect_true(strcmp(attribute_type.data, "AGGREGATE OF INT") == 0, "Unexpected indexed polygonal face attribute type");
    expect_true(ifcopenshell_ifcparse_attribute_value_as_int32_list(indexed_face_indices, &indexed_face_index_values), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_instance_get_argument(texture_vertex_list, 0u, &texture_coords_argument), ifcopenshell_last_error_message());
    expect_true(texture_coords_argument != NULL, "Texture vertex list coordinate handle is null");
    expect_true(ifcopenshell_ifcparse_attribute_value_type(texture_coords_argument, &attribute_type), ifcopenshell_last_error_message());
    expect_true(strcmp(attribute_type.data, "AGGREGATE OF AGGREGATE OF DOUBLE") == 0, "Unexpected texture vertex list attribute type");
    expect_true(ifcopenshell_ifcparse_attribute_value_as_double_list_list(texture_coords_argument, &texture_coord_values), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_instance_get_argument(indexed_polygonal_face_with_voids, 1u, &inner_coord_indices_argument), ifcopenshell_last_error_message());
    expect_true(inner_coord_indices_argument != NULL, "Indexed polygonal face with voids inner index handle is null");
    expect_true(ifcopenshell_ifcparse_attribute_value_type(inner_coord_indices_argument, &attribute_type), ifcopenshell_last_error_message());
    expect_true(strcmp(attribute_type.data, "AGGREGATE OF AGGREGATE OF INT") == 0, "Unexpected inner coordinate index attribute type");
    expect_true(ifcopenshell_ifcparse_attribute_value_as_int32_list_list(inner_coord_indices_argument, &inner_coord_index_values), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifcparse_traverse(polyline, 1, &traversed_instances_value), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifcparse_traverse_breadth_first(polyline, 1, &traversed_instances_bfs_value), ifcopenshell_last_error_message());
    expect_true(traversed_instances_bfs_value.size == 3u, "Unexpected breadth-first traverse result size");

    expect_true(ifcopenshell_ifc_instance_get_argument(colour, 1u, &argument), ifcopenshell_last_error_message());
    expect_true(argument != NULL, "Attribute value handle is null");
    expect_true(ifcopenshell_ifcparse_attribute_value_as_double(argument, &coordinate_value), ifcopenshell_last_error_message());
    expect_true(coordinate_value > 0.099 && coordinate_value < 0.101, "Unexpected colour scalar value");

    expect_true(ifcopenshell_ifc_instance_class_name(traversed, false, &class_name), ifcopenshell_last_error_message());
    expect_true(strcmp(class_name.data, "IfcCartesianPoint") == 0, "Unexpected class name");

    expect_true(ifcopenshell_ifc_file_schema_name(file, &schema_name), ifcopenshell_last_error_message());
    expect_true(strcmp(schema_name.data, "IFC4") == 0, "Unexpected schema name");
    expect_true(ifcopenshell_ifcparse_stream_from_string(IFC_FIXTURE, &streamer), ifcopenshell_last_error_message());
    expect_true(streamer != NULL, "Instance streamer handle is null");
    expect_true(ifcopenshell_ifc_instance_streamer_status(streamer, &streamer_status), ifcopenshell_last_error_message());
    expect_true(streamer_status == 0, "Unexpected instance streamer status");
    expect_true(ifcopenshell_ifc_instance_streamer_has_semicolon(streamer, &streamer_has_semicolon), ifcopenshell_last_error_message());
    expect_true(streamer_has_semicolon, "Expected streamer to detect semicolons");
    expect_true(ifcopenshell_ifc_instance_streamer_semicolon_count(streamer, &attribute_size), ifcopenshell_last_error_message());
    expect_true(attribute_size > 0u, "Expected streamer semicolon count");
    expect_true(ifcopenshell_ifc_instance_streamer_bypassed_instances(streamer, &bypassed_instances), ifcopenshell_last_error_message());
...

    {
        ...
        expect_true(ifcopenshell_ifc_instance_set_argument_double_list(point_two, 0u, &mutated_point_values), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_instance_set_argument_string(person, 1u, "Smith"), ifcopenshell_last_error_message());
    ifcopenshell_ifcparse_attribute_value_destroy(family_name_argument);
    family_name_argument = NULL;
    expect_true(ifcopenshell_ifc_instance_get_argument(person, 1u, &family_name_argument), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifcparse_attribute_value_as_string(family_name_argument, &mutated_family_name), ifcopenshell_last_error_message());
...

    {
        ...
        expect_true(ifcopenshell_ifc_instance_set_argument_string_list(person, 3u, &middle_name_input), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_instance_unset_argument(person, 1u), ifcopenshell_last_error_message());
    ifcopenshell_ifcparse_attribute_value_destroy(family_name_argument);
    family_name_argument = NULL;
    expect_true(ifcopenshell_ifc_instance_get_argument(person, 1u, &family_name_argument), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifcparse_attribute_value_is_null(family_name_argument, &attribute_is_null), ifcopenshell_last_error_message());
...

    {
        ...
        expect_true(ifcopenshell_ifc_instance_set_argument_int32_list(indexed_polygonal_face, 0u, &reversed_index_values), ifcopenshell_last_error_message());
...

    {
        ...
        expect_true(ifcopenshell_ifc_instance_set_argument_double_list_list(texture_vertex_list, 0u, &texture_input), ifcopenshell_last_error_message());
...

    {
        ...
        expect_true(ifcopenshell_ifc_instance_set_argument_int32_list_list(indexed_polygonal_face_with_voids, 1u, &inner_index_input), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_schema_declaration_by_name(schema_lookup, "IfcInteger", &declaration_by_name), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifc_file_create(file, declaration_by_name, -1, &created_integer), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifc_instance_set_argument_int32(created_integer, 0u, 42), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifc_instance_get_argument(created_integer, 0u, &mutated_argument), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifcparse_attribute_value_as_int32(mutated_argument, &wrapped_integer_value), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_schema_declaration_by_name(schema_lookup, "IfcBoolean", &declaration_by_name), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifc_file_create(file, declaration_by_name, -1, &created_boolean), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifc_instance_set_argument_bool(created_boolean, 0u, true), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifc_instance_get_argument(created_boolean, 0u, &mutated_argument), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifcparse_attribute_value_as_bool(mutated_argument, &wrapped_boolean_value), ifcopenshell_last_error_message());
    expect_true(wrapped_boolean_value, "Unexpected mutated wrapped boolean value");
    ifcopenshell_ifcparse_attribute_value_destroy(mutated_argument);
    mutated_argument = NULL;

    expect_true(ifcopenshell_ifc_schema_declaration_by_name(schema_lookup, "IfcReal", &declaration_by_name), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifc_file_create(file, declaration_by_name, -1, &created_real), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifc_instance_set_argument_double(created_real, 0u, 6.25), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifc_instance_get_argument(created_real, 0u, &mutated_argument), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifcparse_attribute_value_as_double(mutated_argument, &wrapped_real_value), ifcopenshell_last_error_message());
    expect_true(wrapped_real_value > 6.249 && wrapped_real_value < 6.251, "Unexpected mutated wrapped real value");
    ifcopenshell_ifcparse_attribute_value_destroy(mutated_argument);
    mutated_argument = NULL;

    expect_true(ifcopenshell_ifc_schema_declaration_by_name(schema_lookup, "IfcActorRole", &declaration_by_name), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifc_file_create(file, declaration_by_name, -1, &created_actor_role), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifc_schema_declaration_by_name(schema_lookup, "IfcRoleEnum", &role_enum_declaration), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifc_declaration_as_enumeration_type(role_enum_declaration, &role_enumeration), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifc_enumeration_lookup_enum_offset(role_enumeration, "ARCHITECT", &architect_role_index), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifc_instance_set_argument_enumeration(created_actor_role, 0u, role_enumeration, architect_role_index), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifc_instance_set_argument_string(created_actor_role, 2u, "Lead architect"), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifc_instance_get_argument(created_actor_role, 0u, &mutated_argument), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifcparse_attribute_value_as_enumeration_value(mutated_argument, &actor_role_name), ifcopenshell_last_error_message());
    expect_true(strcmp(actor_role_name.data, "ARCHITECT") == 0, "Unexpected mutated actor role enumeration");
    ifcopenshell_ifcparse_attribute_value_destroy(mutated_argument);
    mutated_argument = NULL;
    expect_true(ifcopenshell_ifc_instance_get_argument(created_actor_role, 2u, &mutated_argument), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifcparse_attribute_value_as_string(mutated_argument, &actor_role_description), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_schema_declaration_by_name(schema_lookup, "IfcAxis2Placement2D", &declaration_by_name), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifc_file_create(file, declaration_by_name, -1, &created_axis2_placement), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifc_instance_set_argument_instance(created_axis2_placement, 0u, point), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifc_instance_get_argument(created_axis2_placement, 0u, &mutated_argument), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifcparse_attribute_value_as_instance(mutated_argument, &axis_location), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifc_instance_class_name(axis_location, false, &class_name), ifcopenshell_last_error_message());
    expect_true(strcmp(class_name.data, "IfcCartesianPoint") == 0, "Unexpected mutated axis placement location class");
    ifcopenshell_string_destroy(&class_name);
    ifcopenshell_ifcparse_attribute_value_destroy(mutated_argument);
    mutated_argument = NULL;

    expect_true(ifcopenshell_ifc_schema_declaration_by_name(schema_lookup, "IfcPolyline", &declaration_by_name), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifc_file_create(file, declaration_by_name, -1, &created_polyline), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifc_instance_set_argument_instance_list(created_polyline, 0u, polyline_point_list), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifc_instance_get_argument(created_polyline, 0u, &mutated_argument), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifcparse_attribute_value_as_instance_list(mutated_argument, &traversed_instances_bfs), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifcparse_instance_list_size(traversed_instances_bfs, &attribute_size), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_instance_to_string(point_two, true, &instance_spf), ifcopenshell_last_error_message());
    expect_true(strstr(instance_spf.data, "2.5") != NULL, "Mutated point SPF output missing first coordinate");
    ifcopenshell_string_destroy(&instance_spf);

    expect_true(ifcopenshell_ifc_file_write(file, output_path), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifcparse_open(output_path, false, &reopened_file), ifcopenshell_last_error_message());
    expect_true(reopened_file != NULL, "Reopened file handle is null");
    expect_true(ifcopenshell_ifc_file_by_id(reopened_file, 2u, &reopened_point_two), ifcopenshell_last_error_message());
    expect_true(reopened_point_two != NULL, "Reopened point instance handle is null");
    expect_true(ifcopenshell_ifc_file_by_id(reopened_file, 4u, &reopened_person), ifcopenshell_last_error_message());
    expect_true(reopened_person != NULL, "Reopened person instance handle is null");
    expect_true(ifcopenshell_ifc_instance_get_argument(reopened_point_two, 0u, &reopened_point_coordinates), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifcparse_attribute_value_as_double_list(reopened_point_coordinates, &reopened_point_coordinate_values), ifcopenshell_last_error_message());
    ...
    expect_true(ifcopenshell_ifc_instance_get_argument(reopened_person, 1u, &reopened_family_name_argument), ifcopenshell_last_error_message());
    expect_true(ifcopenshell_ifcparse_attribute_value_is_null(reopened_family_name_argument, &reopened_attribute_is_null), ifcopenshell_last_error_message());
    ...
    ifcopenshell_ifc_instance_list_destroy(&instances_by_reference);
    ifcopenshell_ifc_instance_list_destroy(&instances_by_type_excl_subtypes);
    ifcopenshell_ifc_instance_list_destroy(&instances_by_type);
    ...
    traversed = NULL;
    ifcopenshell_ifc_instance_list_destroy(&traversed_instances_bfs_value);
    ifcopenshell_ifc_instance_list_destroy(&traversed_instances_value);
    ifcopenshell_ifcparse_instance_list_destroy(traversed_instances_bfs);
...
static void test_surface(void) {
    ...
    ifcopenshell_ifc_file_t* scratch_file = NULL;
    ...
    ifcopenshell_ifc_instance_list_t inverse_by_decl = {0};
    ifcopenshell_ifc_instance_list_t traverse_list = {0};
    ifcopenshell_ifc_instance_list_t traverse_bfs_list = {0};
    ifcopenshell_ifc_instance_list_t point_instances = {0};
    ifcopenshell_ifc_instance_list_t reopened_point_instances = {0};
    ifcopenshell_ifc_instance_streamer_t* streamer = NULL;
    ...
    expect_ok(ifcopenshell_ifcparse_si_prefix_to_value("KILO", &double_value));
    ...
    expect_ok(ifcopenshell_ifcparse_new_file("IFC4", IFC_FILETYPE_IFCSPF, "", &scratch_file));
    expect_ok(ifcopenshell_ifc_file_create(scratch_file, point_decl, -1, &detached_point));
    ...
    expect_ok(ifcopenshell_ifc_file_add(file, detached_point, -1, &null_instance));
    ...
    ifcopenshell_ifc_instance_list_destroy(&point_instances);
    expect_ok(ifcopenshell_ifc_file_create(file, point_decl, -1, &added_point));
    expect_ok(ifcopenshell_ifc_instance_set_argument_double_list(
        added_point, 0u, &((ifcopenshell_double_list_t){(double[]){4.0, 5.0, 6.0}, 3u})
    ));
    expect_ok(ifcopenshell_ifc_instance_id(added_point, &added_point_id));
    ...
    expect_ok(ifcopenshell_ifc_file_create(file, point_decl, -1, &created_point));
    expect_ok(ifcopenshell_ifc_instance_set_argument_double_list(
    ...
    ifcopenshell_ifc_instance_list_destroy(&reopened_point_instances);
    ifcopenshell_ifc_instance_list_destroy(&traverse_bfs_list);
    ifcopenshell_ifc_instance_list_destroy(&traverse_list);
    ifcopenshell_ifc_instance_list_destroy(&inverse_by_decl);
    ifcopenshell_ifc_file_destroy(scratch_file);
    unlink(output_path);
}
...
static void test_rocksdb(void) {
    ...
    ifcopenshell_ifc_instance_list_t point_instances_by_type = {0};
    ifcopenshell_ifc_instance_list_t inverse_instances = {0};
    ifcopenshell_ifcparse_attribute_value_t* point_coordinates = NULL;
    ifcopenshell_ifcparse_attribute_value_t* polyline_points = NULL;
    ifcopenshell_ifcparse_attribute_value_t* family_name_argument = NULL;
    ifcopenshell_ifcparse_attribute_value_t* middle_names_argument = NULL;
    ...
    expect_ok(ifcopenshell_ifc_file_create(file, point_decl, -1, &point_one));
    expect_ok(ifcopenshell_ifc_file_create(file, point_decl, -1, &point_two));
    expect_ok(ifcopenshell_ifc_file_create(file, polyline_decl, -1, &polyline));
    expect_ok(ifcopenshell_ifc_file_create(file, person_decl, -1, &person));
    ...
    expect_ok(ifcopenshell_ifc_file_by_type(file, "IfcCartesianPoint", &point_instances_by_type));
    expect_ok(ifcopenshell_ifcparse_instance_list_create_from_handles(&point_instances_by_type, &point_instances));
    ...
    ifcopenshell_ifc_instance_list_destroy(&point_instances_by_type);
    ...
    ifcopenshell_ifc_instance_list_destroy(&inverse_instances);
